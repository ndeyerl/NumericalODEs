...
public:
  ...
  int Evaluate(std::vector<double>& y, std::vector<double>& resid) {
    ...
    // combine pieces to fill residual, y-yold-h*[theta*f(t+h,y)+(1-theta)*f(t,yold)]
...
// General Trap. residual Jacobian function class -- implements 
// a backward-Euler-specific ResidualJacobian to be supplied 
// to the Newton solver.
class TrapResidJac: public ResidualJacobian {
public:

  // data required to evaluate General Trap. residual Jacobian
...
class TrapezoidalStepper {
 ...
 private:
  ...
  TrapResid *resid;           // general trap. residual function pointer
  TrapResidJac *residJac;     // general trap. residual Jacobian function pointer

 public:

  NewtonSolver *newt;   // Newton nonlinear solver pointer

  // constructor (constructs residual, Jacobian, theta, and copies y for local data)
  TrapezoidalStepper(RHSFunction& frhs_, RHSJacobian& Jrhs_, double theta,
